...
IMAGE_UPLOAD_FOLDER = os.getenv("IMAGE_UPLOAD_FOLDER", "/tmp/product_images")
...
def init_db():
    ...
    add_column_if_missing(c, "categories", "image_file", "image_file TEXT")
    add_column_if_missing(c, "categories", "image_url", "image_url TEXT")
...
def save_product_image(file_obj):
    ...
    if not allowed_image(file_obj.filename):
        flash("Only PNG, JPG, JPEG, WEBP and GIF product images are allowed.")
        return None
    safe = secure_filename(file_obj.filename)
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    filename = f"{timestamp}_{safe}"
    file_obj.save(os.path.join(app.config["IMAGE_UPLOAD_FOLDER"], filename))
    return filename

def category_image_src(c):
    if c and "image_url" in c.keys() and c["image_url"]:
        return c["image_url"]
    if c and "image_file" in c.keys() and c["image_file"]:
        return url_for("product_image", filename=c["image_file"])
    return ""
...
{% if not user %}<div id="loginPop" class="popup"><div class="modal reveal"><span class="pill">Exclusive Access</span><h2>{{ settings.get('popup_title') }}</h2><p class="muted">{{ settings.get('popup_body') }}</p>{% if active_offer %}<p class="pill offer">{{ active_offer.discount }}% offer valid till {{ active_offer.deadline }}</p>{% endif %}<div style="display:flex;gap:10px;flex-wrap:wrap"><a class="btn" href="{{ url_for('signup') }}">Create Account</a><a class="btn secondary" href="{{ url_for('login') }}">Login</a><button class="btn secondary" onclick="closePop()">Continue Browsing</button></div></div></div>{% endif %}<script>function toggleMainMenu(){const m=document.getElementById('mobileMenu');if(m){m.classList.toggle('show')}}function toggleAdminMenu(){const m=document.getElementById('adminMobileMenu');if(m){m.classList.toggle('show')}}document.addEventListener('click',function(e){const menu=document.getElementById('mobileMenu');const btn=document.querySelector('.menu-btn');if(menu&&btn&&!menu.contains(e.target)&&!btn.contains(e.target)){menu.classList.remove('show')}});{% if not user %}setTimeout(()=>{const p=document.getElementById('loginPop');if(p){p.classList.add('show')}},1900);function closePop(){document.getElementById('loginPop').classList.remove('show')}{% endif %}</script>
</body></html>
"""
...
<section class="section"><div class="wrap"><h2>Most viewed categories</h2><div class="cards">{% for c in cats %}<div class="card cat">{% set img = category_image_src(c) %}{% if img %}<img class="product-img" src="{{ img }}" alt="{{ c.name }} image">{% endif %}<span class="tag">{{ c.type }}</span><h3>{{ c.name }}</h3><p class="muted">{{ c.description }}</p><p><span class="price">₹{{ c.price }}</span> <span class="muted">starting</span></p>{% if c.offer_deadline %}<p class="pill offer">Offer valid till {{ c.offer_deadline }}</p>{% endif %}<a class="btn secondary" href="{{ url_for('category_detail', slug=c.slug) }}">View Demo</a></div>{% endfor %}</div></div></section>
...
def admin_layout(content):
    return page(render_template_string(r"""<section class="section"><div class="wrap"><button class="btn admin-menu-btn" type="button" onclick="toggleAdminMenu()">☰ Admin Menu</button><div id="adminMobileMenu" class="card side admin-mobile-menu"><h3>Admin Panel</h3><a href="{{url_for('admin')}}">Dashboard</a><a href="{{url_for('admin_categories')}}">Manage Categories</a><a href="{{url_for('admin_customers')}}">Customers</a><a href="{{url_for('admin_payments')}}">Payments</a><a href="{{url_for('admin_support')}}">Support Tickets</a><a href="{{url_for('admin_ads')}}">Advertisements</a><a href="{{url_for('admin_offers')}}">Discount Offers</a><a href="{{url_for('admin_settings')}}">Settings</a></div></div><div class="wrap admin"><div class="card side"><h3>Admin Panel</h3><a href="{{url_for('admin')}}">Dashboard</a><a href="{{url_for('admin_categories')}}">Manage Categories</a><a href="{{url_for('admin_customers')}}">Customers</a><a href="{{url_for('admin_payments')}}">Payments</a><a href="{{url_for('admin_support')}}">Support Tickets</a><a href="{{url_for('admin_ads')}}">Advertisements</a><a href="{{url_for('admin_offers')}}">Discount Offers</a><a href="{{url_for('admin_settings')}}">Settings</a></div><div>{{content|safe}}</div></div></section>""", content=content), "Admin")
...
def admin_categories():
    ...
    if request.method == "POST":
        ...
        image_file = uploaded_image or (existing["image_file"] if existing and "image_file" in existing.keys() else None)
        image_url = request.form.get("image_url") or (existing["image_url"] if existing and "image_url" in existing.keys() else None)
        data=(request.form["name"], request.form["slug"], request.form["type"], request.form["description"], request.form["price"], request.form["discount"], request.form["demo_data"], request.form.get("demo_link"), demo_file, request.form.get("offer_deadline"), image_file, image_url, 1 if request.form.get("active") else 0)
        if cid:
            query("UPDATE categories SET name=?,slug=?,type=?,description=?,price=?,discount=?,demo_data=?,demo_link=?,demo_file=?,offer_deadline=?,image_file=?,image_url=?,active=? WHERE id=?", (*data,cid), commit=True)
        else:
            query("INSERT INTO categories(name,slug,type,description,price,discount,demo_data,demo_link,demo_file,offer_deadline,image_file,image_url,active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", data, commit=True)
